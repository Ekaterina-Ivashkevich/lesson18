...
function wordsList(str, subStr) {
    ...
    while (delChars.test(str)) {
        str = str.replace(delChars, '');
    }
...
function getLocalDate(date, isSeconds = false, isISO = false) {
    function format(d) {
        const dtf = new Intl.DateTimeFormat('ru', d);
    ...
    let dayFormat = isISO ? [{ year: 'numeric' }, { month: 'numeric' }, { day: 'numeric' }] : [{ day: 'numeric' }, { month: 'numeric' }, { year: 'numeric' }];
    let timeFormat = [{ hour: 'numeric' }, { minute: 'numeric' }];
    if (isSeconds) {
        timeFormat.push({ second: 'numeric' });
    }
    let result = dayFormat.map(format).join(isISO ? '-' : '.');
    result += ', ' + timeFormat.map(format).join(':');
    return result;
}
...
function getDateAgo(date, days) {
    ...
    return dateNew.toLocaleString().replace(/(\d.*),\s+(\d.*)/gu, '$1');
};
